...
class minrv(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # [Mod1] Set the riscv_prefix in config.ini
        ...
        # [Mod2] Set the path to minrv executable
        self.dut_exe = os.path.abspath(config['path_to_model'])
        # self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","minrv")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       # runTests function
       # [Mod3] Set the compile command for riscv-none-elf-gcc toolchain
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for minrv start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
	...
	#   # echo statement.
    #       if self.target_run:
    #         # set up the simulation command. Template is for minrv. Please change.
...
#The following template only uses shell commands to compile and run the tests.
    ...
    # [Mod4] Uncomment the following function to use shell commands
    def runTests(self, testList):
        ...
        for testname in testList:
            ...
            if self.target_run:
                # build the command for running the elf on the DUT. In this case we use minrv and indicate
                # the isa arg that we parsed in the build stage, elf filename and signature filename.
                # Template is for minrv. Please change for your DUT
                
                # [Mod5] ELF to BIN conversion
                objcopy_cmd = f'{self.riscv_prefix}objcopy -O binary {elf} {bin_file}'
                utils.shellCommand(objcopy_cmd).run(cwd=test_dir)

                # [Mod6] Symbol extraction for signature addresses
                nm_cmd = f'{self.riscv_prefix}nm {elf} | grep -E "begin_signature|end_signature|tohost"'
                sig_start, sig_end, tohost = "0", "0", "0"
                try:
                    output = subprocess.check_output(nm_cmd, cwd=test_dir, shell=True).decode('utf-8')
                    for line in output.splitlines():
                        parts = line.split()
                        if len(parts) >= 1:
                            if "begin_signature" in line: sig_start = "0x" + parts[0]
                            if "end_signature" in line: sig_end = "0x" + parts[0]
                            if "tohost" in line: tohost = "0x" + parts[0]
                except subprocess.CalledProcessError:
                    logger.error(f"Failed to extract symbols from {elf}")
                
                # format: <simulator> <bin_file> <sig_file> <start> <end> <tohost>
                # [Mod7] Execution command with formatted arguments
                execute = f'{self.dut_exe} {bin_file} {sig_file} {sig_start} {sig_end} {tohost}'
                logger.debug('Executing on minrv ' + execute)
