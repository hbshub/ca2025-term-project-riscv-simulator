...
static inst_handler_t dispatch_table[RV32I_INVALID + 1];


// default invalid handler
static vm_step_result_t invalid_handler(cpu_t *cpu, rv32i_instruction inst, uint32_t raw, uint32_t pc) {
...
vm_step_result_t riscv_execute(cpu_t *cpu, uint32_t raw, uint32_t pc_current)
{
    ...
    rv32i_instruction inst_id = rv32i_decode(&raw);
